...
export default function Withdraw() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-xl">
      ...
      {/* Warning box */}
      <div className="rounded-xl p-5 mb-6 flex flex-col gap-3" style={{ backgroundColor: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.2)" }}>
        <div className="flex items-center gap-2">
          <AlertTriangle size={16} style={{ color: "#EF4444" }} />
          <span className="text-sm font-semibold" style={{ color: "#DC2626" }}>탈퇴 시 주의사항</span>
        </div>
        <ul className="flex flex-col gap-1.5">
          {[
            ...
            <li key={item} className="flex items-start gap-2 text-sm" style={{ color: "#7F1D1D" }}>
              <ChevronRight size={13} className="flex-shrink-0 mt-0.5" />{item}
            </li>
          ))}
        </ul>
      </div>
              ...
              {REASONS.map(([value, label]) => (
                <label key={value}
                  ...
                  <div className={cn(
                    ...
                    {reason === value && (
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: "#0D3B6E" }} />
                    )}
                  </div>
                  <input type="radio" name="reason" value={value} className="sr-only"
        ...
        <div className="flex flex-col gap-5">
          <div className="bg-white border border-border rounded-xl p-5 flex flex-col gap-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-foreground">{reason ? WITHDRAWAL_REASON_LABELS[reason as WithdrawalReason] : "-"}</span>
            </div>
            {reasonDetail && (
              <div className="flex justify-between text-sm">
