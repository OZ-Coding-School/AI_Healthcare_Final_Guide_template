  ...
  const handleSendMail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof ApiError
        ? (err.status === 404 ? "등록되지 않은 이메일입니다." : err.message)
      ...
      <div className="hidden lg:flex flex-col justify-between w-[440px] flex-shrink-0 p-12" style={{ backgroundColor: "#0B1628" }}>
        ...
        <div className="flex flex-col gap-5">
          <h2 className="text-4xl font-normal text-white leading-snug" style={{ fontFamily: "Instrument Serif, serif" }}>
            계정을<br /><span style={{ color: "#10B981" }}>복구합니다</span>
          </h2>
          <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>
            가입 시 사용한 이메일 주소로 인증 코드를 발송합니다. 코드 확인 후 계정 복구가 완료됩니다.
          </p>
          {/* Steps */}
            ...
            {[
              ...
              <div key={num} className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                  style={{ backgroundColor: done ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.07)", color: done ? "#10B981" : "rgba(255,255,255,0.4)", fontFamily: "JetBrains Mono" }}>
                  {done ? <CheckCircle size={14} /> : num}
                </div>
                <span className="text-sm" style={{ color: done ? "rgba(255,255,255,0.8)" : "rgba(255,255,255,0.4)" }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {step === "email" && (
            <>
              <div className="mb-8">
                ...
                <p className="text-sm text-muted-foreground">가입 시 사용한 이메일로 인증 코드를 받으세요.</p>
              </div>
              ...
              <p className="mt-6 text-sm text-center text-muted-foreground">
                <Link to="/login" className="flex items-center justify-center gap-1.5 hover:text-foreground transition-colors">
            ...
            <>
              <div className="mb-8">
                ...
                <p className="text-sm text-muted-foreground">
                  <span className="font-medium text-foreground">{email}</span>로 발송된<br />6자리 인증 코드를 입력해주세요.
                </p>
              </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <input type="text" inputMode="numeric" maxLength={6} placeholder="123456"
                      ...
                      className={cn(INPUT_BASE, "pr-24 text-center tracking-widest text-lg font-semibold")}
                      style={{ borderColor: codeError ? "#EF4444" : "rgba(13,59,110,0.2)", fontFamily: "JetBrains Mono" }} />
                    {timer > 0 && (
                  ...
                  {timer === 0 && (
                    <div className="flex items-center justify-between text-xs mt-1">
                      ...
                      <button type="button" onClick={handleResend} disabled={sending}
                        className="font-medium hover:underline disabled:opacity-50" style={{ color: "#0D3B6E" }}>
            ...
            <div className="flex flex-col items-center text-center gap-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  계정이 성공적으로 복구되었습니다.<br />새로운 비밀번호로 로그인해주세요.
                </p>
              </div>
